...
HRESULT CGraphic_Device::InitDevice(const GRAPHICDESC& GraphicDesc, LPDIRECT3DDEVICE9* ppOut)
{
	...
	{
		MSX_BOX("GetDeviceCaps Failed"); // mfc에서만 사용가능한 메세지 박스 함수
		return E_FAIL;
	}
	...
	{
		MSX_BOX("CreateDevice Failed"); // mfc에서만 사용가능한 메세지 박스 함수
		return E_FAIL;
	}

	if (FAILED(D3DXCreateSprite(m_pDevice, &m_pSprite)))
	{
		MSX_BOX("m_pSprite Failed");
		return E_FAIL;
	}
...
void CGraphic_Device::Free()
{
	...
	if (0 != Safe_Release(m_pDevice))
		MSX_BOX("Failed to Release : IDirect3DDevice9");

	if (0 != Safe_Release(m_p3D))
		MSX_BOX("Failed to Release : IDirect3D9");
}
